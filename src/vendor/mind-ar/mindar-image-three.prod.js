// physgame2: 브라우저 전용 번들이므로 Node 전용 폴백 경로(require)는 도달하지 않는다.
// Vite dev 서버 의존성 스캐너 오탐을 막기 위해 리터럴 require(...) 호출을 헬퍼로 치환했다.
const __nodeOnlyRequire = (name) => { throw new Error("이 브라우저 번들에서는 Node 전용 모듈을 사용할 수 없다: " + name); };
...
class Vb extends tn {
  constructor(e) {
    if (super(), this.upstream = e, P().get("IS_BROWSER"))
      ...
      const { StringDecoder: t } = __nodeOnlyRequire("string_decoder");
